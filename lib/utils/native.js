...
function findClassNameBindings(classDeclaration) {
  ...
  for (let binding of findStringArrayDecorator(classDeclaration, 'classNameBindings')) {
    ...
    let [property, truthy, falsy] = parts;

    if (parts.length === 1) {
      let type = getTypeOfProperty(classDeclaration, property);
      if (type === 'BooleanLiteral') {
        classNameBindings.set(property, [_.kebabCase(property), null]);
      } else if (type === 'StringLiteral') {
        classNameBindings.set(property, 'string');
      } else {
        classNameBindings.set(property, 'string');
        console.warn(
          `Cannot determine type of property "${property}" used with classNameBindings. Assuming a string type, but double check!`
        );
      }
    } else if (parts.length === 2) {
      classNameBindings.set(property, [truthy, null]);
    } else if (parts.length === 3) {
      classNameBindings.set(property, [truthy || null, falsy]);
    } else {
...
function getTypeOfProperty(classDeclaration, propertyName) {
  let properties = j(classDeclaration).find(j.ClassProperty, {
    key: {
      type: 'Identifier',
      name: propertyName,
    },
  ...
  let property = properties.get().node;
