...
describe('classic components', function() {
  ...
  test('handles boolean `classNameBindings` correctly', () => {
    let source = `
    export default Component.extend({
...
describe('native components', function() {
  ...
  test('handles `@classNameBindings` based on type correctly', () => {
    let source = `
      import Component from '@ember/component';
      import { classNameBindings } from '@ember-decorators/component';

      @classNameBindings('foo', 'bar', 'baz', 'isSelected', 'customClass')
      export default class FooComponent extends Component {
        foo = true;
        bar = 'hey';
        baz;
